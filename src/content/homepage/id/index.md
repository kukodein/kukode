---
title: Beranda
description: Kukode Digital Technology memberikan layanan untuk meningkatkan ekspansi perusahaan & kehadiran online Anda.
seo_title: Kukode Digital Technology - Tingkatkan Web & Bisnis Anda
seo_description: Kukode Digital Technology memberikan layanan untuk meningkatkan ekspansi perusahaan & kehadiran online Anda. Lihat solusi kami untuk meningkatkan website Anda.
seo_image: /image/default-thumbnail.jpg

hero:
  badge_text: Menerima Proyek Baru
  title: Kami membangun website yang benar-benar bekerja untuk bisnis Anda.
  description: Bukan sekadar tampilan menarik — setiap website yang kami buat cepat, profesional, dan dirancang untuk mengonversi. Anda berkomunikasi langsung dengan developer, dengan Kami, Kukode. Tanpa perantara, tanpa tebak-tebakan.
  cta_text: Hubungi Kami
  cta_link: "#contact"
  secondary_cta_text: Lihat Karya Kami
  secondary_cta_link: "#portfolio"
  image: /image/en-home-section-hero.png
  stat_1: ⚡ Skor kecepatan rata-rata 98/100
  stat_2: 🔍 SEO-ready sejak hari pertama

trusted:
  title: Bisnis yang telah kami bantu go online
  items:
    - Acme
    - Northwind
    - Globex
    - Initech
    - Umbrella
    - Soylent

marquee:
  items:
    - Company Profile
    - Landing Page
    - Toko Online
    - Redesign Website
    - WordPress
    - Next.js
    - UI/UX Figma
    - WooCommerce
    - SEO Ready
    - Mobile First

numbers:
  items:
    - number: 120+
      label: Website diluncurkan
    - number: "98"
      label: Rata-rata skor kecepatan
    - number: 5thn
      label: Membangun website
    - number: 99%
      label: Klien ingin bekerja sama lagi

about:
  eyebrow: Tentang Kukode
  heading: Tim pengembangan web yang berpikir seperti pemilik bisnis.
  p1: Kami tidak hanya menulis kode. Kami memastikan setiap website adalah alat bisnis yang nyata. Setiap keputusan desain dan teknis dibuat dengan mempertimbangkan pengguna akhir dan target pertumbuhan Anda, bukan sekadar apa yang terlihat bagus di layar.
  p2: Berbasis di Indonesia, siap kerja remote, dan berkomitmen penuh pada setiap proyek. Anda berhubungan langsung dengan Kami — pihak yang merancang, mengode, dan menyelesaikan situs Anda. Tanpa sales rep. Tanpa account manager. Tanpa kejutan di akhir proyek.
  image: /image/en-home-section-tentang.png
  cta_text: Pelajari lebih lanjut tentang kami
  cta_link: /id/about/

services:
  eyebrow: Layanan
  heading: Apa yang bisa Kami bantu untuk bisnis Anda?
  description: Dari website baru hingga pembaruan menyeluruh pada website yang sudah ada — setiap proyek kami bangun dengan standar tinggi yang sama.
  items:
    - icon: 💻
      title: Pengembangan Web
      description: Website dibuat khusus secara manual, dikode untuk memuat sangat cepat.
      col_class: col-md-8
    - icon: 🎨
      title: Desain Web
      description: Desain khusus yang mencerminkan identitas brand Anda.
      col_class: col-md-4
    - icon: 🛒
      title: Toko Online
      description: Website e-commerce siap menerima pesanan dan sistem pembayaran otomatis.
      col_class: col-md-4
    - icon: 🔍
      title: Optimasi SEO
      description: SEO on-page yang terpasang rapi sejak hari pertama pembuatan.
      col_class: col-md-4
    - icon: 🛠️
      title: Pemeliharaan Website
      description: Pembaruan rutin, monitoring performa, dan perbaikan berkelanjutan.
      col_class: col-md-4
    - icon: 🚀
      title: Hosting & Deployment
      description: Penyiapan hosting yang cepat, aman, dan sangat andal.
      col_class: col-md-4
    - icon: 📱
      title: Ramah Ponsel (Mobile-First)
      description: Dibangun untuk tampil dan bekerja sempurna di smartphone.
      col_class: col-md-4
    - icon: 📈
      title: Analitik & Pelacakan
      description: Ketahui dengan tepat bagaimana pengunjung menemukan dan menggunakan situs Anda.
      col_class: col-md-4

process:
  eyebrow: Cara Kami Bekerja
  heading: Sederhana, transparan, tanpa drama.
  description: Dari percakapan pertama hingga hari peluncuran — Anda selalu tahu persis sejauh mana perkembangan proyek Anda.
  items:
    - step_number: "1"
      title: Diskusi & Riset
      description: Kita mendiskusikan bisnis, target Anda, dan apa yang masih kurang dari situs Anda saat ini.
    - step_number: "2"
      title: Perancangan Desain
      description: Anda akan melihat wireframe terlebih dahulu, lalu desain visual lengkap yang sesuai dengan brand.
    - step_number: "3"
      title: Pengembangan Kode
      description: Kami membangun situs nyata — cepat, responsif, dan siap untuk mesin pencari.
    - step_number: "4"
      title: Pengujian & Review
      description: Kami menguji di berbagai perangkat, memeriksa kecepatan dan SEO, lalu mendemonstrasikannya bersama Anda.
    - step_number: "5"
      title: Peluncuran & Dukungan
      description: Situs Anda online, dan kami tetap siap membantu menangani kebutuhan selanjutnya.

portfolio:
  eyebrow: Karya Kami
  heading: Hasil karya yang berbicara sendiri.
  cta_text: Lihat Semua Proyek
  cta_link: /id/portfolio/
  items:
    - title: Proyek Satu
      category: Profil Perusahaan
      image: https://picsum.photos/600/400?41
      link: "#"
    - title: Proyek Dua
      category: E-Commerce
      image: https://picsum.photos/600/400?42
      link: "#"
    - title: Proyek Tiga
      category: Landing Page
      image: https://picsum.photos/600/400?43
      link: "#"
    - title: Proyek Empat
      category: Blog
      image: https://picsum.photos/600/400?44
      link: "#"
    - title: Proyek Lima
      category: Portofolio
      image: https://picsum.photos/600/400?45
      link: "#"
    - title: Proyek Enam
      category: Aplikasi Web
      image: https://picsum.photos/600/400?46
      link: "#"

stack:
  eyebrow: Tech Stack
  heading: Dibangun dengan teknologi yang tepat.
  description: Tidak asal ikut tren. Setiap alat dipilih karena terbukti memberikan hasil terbaik bagi bisnis klien.
  categories:
    - name: Frontend
      tools:
        - HTML5
        - CSS3 / SCSS
        - JavaScript
        - Tailwind CSS
        - Next.js
    - name: Backend & CMS
      tools:
        - WordPress
        - Elementor Pro
        - WooCommerce
        - PHP 8
    - name: Desain & Alat
      tools:
        - Figma
        - Canva Pro
        - Git / GitHub
        - Yoast SEO
        - Cloudflare
    - name: Pembayaran & API
      tools:
        - Midtrans
        - Xendit
        - WhatsApp API
    - name: Analitik & SEO
      tools:
        - Google Analytics
        - Search Console
        - PageSpeed
    - name: Hosting
      tools:
        - Niagahoster
        - Hostinger
        - cPanel

testimonials:
  eyebrow: Testimoni
  heading: Langsung dari orang-orang yang telah bekerja sama dengan kami.
  items:
    - quote: Website baru kami memuat kurang dari satu detik dan pertanyaan masuk meningkat dua kali lipat di bulan pertama.
      icon: 👨‍💼
      name: Budi Santoso
      role: CEO, Perusahaan A
    - quote: Saya bisa memperbarui website sendiri tanpa harus memanggil orang lain — itu saja sudah sangat berharga.
      icon: 👩
      name: Sari Dewi
      role: Pemilik, Toko B
    - quote: Mereka benar-benar menjelaskan apa yang sedang dikerjakan daripada hanya sekadar mengirim tagihan.
      icon: 👨‍⚕️
      name: Andi Wijaya
      role: Marketing, Startup C
    - quote: Akhirnya kami punya website yang terlihat seperti milik tahun 2026, bukan tahun 2015.
      icon: 👩
      name: Rina Putri
      role: Pendiri, Brand D

why_us:
  eyebrow: Mengapa Kukode
  heading: Inilah yang membuat kami benar-benar berbeda.
  description: Delapan alasan mengapa klien selalu kembali untuk proyek mereka berikutnya.
  items:
    - icon: 🎯
      title: Akses langsung ke developer Anda
      description: Tanpa perantara sales rep atau project manager.
    - icon: ⚡
      title: Diluncurkan dalam hitungan minggu
      description: Proses kerja yang efisien membuat website Anda siap dalam hitungan minggu, bukan bulan.
    - icon: 🔒
      title: Aman secara bawaan
      description: Setup hosting modern meminimalisir masalah celah keamanan.
    - icon: 🎯
      title: Dibangun sesuai tujuan bisnis Anda
      description: Setiap situs dibuat mengitari konten dan target pertumbuhan Anda.
    - icon: 💬
      title: Komunikasi yang jelas
      description: Pembaruan rutin sehingga Anda selalu mengetahui progres proyek.
    - icon: 📊
      title: Ramah SEO
      description: Dibangun dengan standar teknis SEO terbaik sejak awal.
    - icon: 🔄
      title: Revisi fleksibel
      description: Ruang untuk iterasi agar hasil akhir sesuai dengan visi Anda.
    - icon: 💰
      title: Harga transparan
      description: Tanpa biaya tersembunyi — apa yang disepakati adalah apa yang Anda bayar.
    - icon: 🤝
      title: Dukungan pasca-peluncuran
      description: Kami tetap hadir setelah rilis untuk membantu Anda berkembang.

pricing:
  eyebrow: Harga
  heading: Investasi yang benar-benar masuk akal.
  description: Tidak ada proyek yang terlalu kecil atau terlalu besar. Semua disesuaikan dengan kebutuhan nyata Anda.
  plans:
    - name: Starter
      is_popular: false
      price: IDR 800K
      approx: ≈ USD 50 / proyek
      description: Untuk bisnis yang baru memulai langkah di dunia digital.
      features:
        - Halaman tunggal landing page
        - Desain responsif penuh
        - Tombol WhatsApp & formulir kontak
        - Mockup Figma sebelum koding
        - Selesai dalam 3–5 hari kerja
      cta_text: Konsultasi Sekarang
      cta_link: "#contact"
    - name: Professional
      is_popular: true
      badge: Paling populer
      price: IDR 1.5M
      approx: ≈ USD 95 / proyek
      description: Untuk bisnis yang ingin tampil profesional dan membangun kepercayaan klien.
      features:
        - 4–6 halaman profil perusahaan
        - Mockup Figma + revisi desain
        - Setup dasar SEO on-page
        - Konfigurasi Google Analytics
        - Revisi terbuka sesuai ruang lingkup
        - Selesai dalam 7–10 hari kerja
      cta_text: Pilih Paket Ini
      cta_link: "#contact"
    - name: E-Commerce
      is_popular: false
      price: IDR 2.5M
      approx: ≈ USD 158 / proyek
      description: Untuk bisnis yang siap berjualan online — tanpa potongan komisi.
      features:
        - Setup toko WooCommerce lengkap
        - Manajemen katalog & inventaris produk
        - Integrasi payment gateway
        - Kalkulator ongkir otomatis
        - Termasuk pelatihan panel admin
      cta_text: Konsultasi Sekarang
      cta_link: "#contact"

faq:
  eyebrow: FAQ
  heading: Pertanyaan yang sering kami dengar.
  description: Tidak menemukan pertanyaan Anda di sini? Silakan tanya langsung — Kami membalas secara pribadi setiap pesan.
  cta_text: Chat Kami Langsung
  cta_link: https://wa.me/62853293353541
  items:
    - question: Berapa lama waktu pengerjaan satu proyek?
      answer: Biasanya 2–4 minggu tergantung pada tingkat kerumitan.
    - question: Bisakah saya meminta desain kustom?
      answer: Tentu saja, kami merancang setiap desain khusus sesuai brand Anda.
    - question: Bagaimana sistem pembayarannya?
      answer: Uang muka 50% di awal, pelunasan setelah proyek selesai.
    - question: Apakah hosting sudah termasuk?
      answer: Tergantung pada paket — silakan lihat detail pada tabel Harga.
    - question: Berapa kali revisi yang diberikan?
      answer: Biasanya 2–3 putaran revisi, tergantung pada paket yang dipilih.
    - question: Apakah ada bantuan setelah website selesai?
      answer: Ya, masa garansi dan dukungan teknis tersedia sesuai paket.
    - question: Bagaimana cara memulainya?
      answer: Hubungi kami melalui formulir kontak atau WhatsApp di bawah.
    - question: Bisakah migrasi dari atau ke WordPress?
      answer: Ya, itu adalah salah satu keahlian utama kami.

cta:
  eyebrow: Mari Bicara
  heading: Siap memulai? Mari ngobrol terlebih dahulu.
  description: Konsultasi gratis tanpa paksaan. Ceritakan bisnis Anda dan kami akan mencarikan solusi terbaik bersama.
  primary_text: Hubungi Kami
  primary_link: /id/contact/
  secondary_text: Chat WhatsApp
  secondary_link: https://wa.me/62853293353541
---
